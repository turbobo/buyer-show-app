...
每次代码改动**必须**在同一次提交中同步对应文档：

| 改动类别 | 目标文档 |
|---------|---------|
| 架构调整、技术选型、模块分层、目录结构、部署/构建链路 | `技术方案文档.md` |
| 数据库 schema、表结构、索引、RLS、触发器、SQL 迁移 | `技术方案文档.md` |
| 关键代码设计：状态管理、路由约定、API 契约、抽象/封装模式 | `技术方案文档.md` |
| 新增/修改/删除用户可感知功能（页面、交互、按钮、流程） | `产品设计文档.md` |
| 待办（TODO/Roadmap）项的增删与状态变更 | `产品设计文档.md` |

**操作要点**：

- 一项改动可能同时命中两类，需同步两份文档（如「新增收藏功能」既要在产品文档登记功能项，又要在技术文档登记 favorites 表设计）。
- 文档变更与代码变更**同一次 commit 内**完成；按功能拆多次 commit 时，文档更新归入对应那次 feat/fix commit，不要单独凑「docs: 更新文档」。
- 待办项完成时从「待办」段移到「已完成」段（或勾选 `[x]`），不要静默删除。
- 不要在文档中粘贴整段代码 diff —— 总结设计决策、关键字段/接口签名即可。
- 不要创建第三份文档来绕过本规则。

**提交前自检**：本次 `git diff --stat` 中若包含 `src/`、`supabase/`、`next.config`、`tailwind.config` 等代码或配置文件，必须同时包含 `技术方案文档.md` 或 `产品设计文档.md` 中至少一项的修改。
